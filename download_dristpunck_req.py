...
def download_and_update_mods():
    ...
    print(Back.YELLOW + f"Download mods from Github status: in progress (files path: {destination_folder}",
          Style.RESET_ALL)
...
def minecraft_download_dristpunk():
    ...
    print(Back.YELLOW + f"Download minecraft from github status: in progress (files path: {destination_folder}",
          Style.RESET_ALL)
    ...
    response = requests.get(repo_url, stream=True)
    print(Back.GREEN + "Download minecraft.zip status: Done", Style.RESET_ALL)
    print("\n")
